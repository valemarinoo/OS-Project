...
static ssize_t CC_decrypt(const int * input, int * output, size_t len) {
	...
	//char i_input[len], i_output[len];
	unsigned long i_o[100];
	if(copy_from_user((unsigned int *)INPUT_START, (unsigned long *)input, sizeof(unsigned long)*len))
		return -2;


	void __iomem *io;
	io = ioremap(STATUS_ID, 8);
	iowrite64((unsigned long)2, io);

	if(copy_to_user((unsigned int *)OUTPUT_START, i_o, sizeof(unsigned long)*len))
		return -3;
